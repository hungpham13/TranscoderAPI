...
namespace FFMpegCore
{
    public class FFMpegArgumentProcessor
    {
        ...
        public async Task<bool> ProcessAsynchronously( bool throwOnError = true, CancellationTokenSource? inputCSource=null, FFOptions? ffMpegOptions = null)
        {
            var options = GetConfiguredOptions(ffMpegOptions);
            var processArguments = PrepareProcessArguments(options, out var cancellationTokenSource);
            ...
            try
            {
                processResult = await Process(processArguments, cSource).ConfigureAwait(false);
            }
            catch (OperationCanceledException)
            {
                if (throwOnError)
